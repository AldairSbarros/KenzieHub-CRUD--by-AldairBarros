    ...
    <ul className={style.listTech}>
      {user.techs.map((tech) => (
        <li className={style.listTechItem} key={tech.id}>
          ...
          <button className={style.icons} onClick={() =>setEditingTech(tech.id)}><img src={penedit} ></img></button>
          <button className={style.icons} onClick={() => deleteTech(tech.id)}> <img src={deleticon} alt="" /> </button>
